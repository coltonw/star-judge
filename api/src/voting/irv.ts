import { GRADES, type Grade, type Candidate, type RankedCandidate, type Vote } from '../db/types'
...
export function rankIrv(candidates: Candidate[], votes: Vote[]): RankedCandidate[] {
  ...
  if (votes.length === 0) {
    return candidates.map((c, i) => ({
      ...c, rank: i + 1, gradeCounts: gradeCounts[c.id], totalVotes: 0,
    }))
  }
  ...
  const maxRound = Math.max(0, ...Object.values(elimRound))
